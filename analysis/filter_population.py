        ...
        df_filtered = df[
            (
                ...
                & (
                    ((df["age"] >= 65) & (df["ppi"] == 0))
                    | (
                        (df["methotrexate_6_3_months"] == 1)
                        & (df["methotrexate_3_months"] == 1)
                    )
                    | ((df["lithium_6_3_months"] == 1) & (df["lithium_3_months"] == 1))
                    | (
                        (df["amiodarone_12_6_months"] == 1)
                        & (df["amiodarone_6_months"] == 1)
                    )
                    | (
                        ((df["gi_bleed"] == 1) | (df["peptic_ulcer"] == 1))
                        & (df["ppi"] == 0)
                    )
                    | (df["anticoagulant"] == 1)
