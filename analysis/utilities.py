...
def compare_indicator_k_columns(directory: str) -> None:
    ...
    new_k_dict = dict(
        zip(new_k_df["patient_id"], new_k_df["indicator_k_denominator_new"])
    )
    ...
    for file in filelist:
        ...
        if match_input_files(file.name):
            ...
            denominator_match_counts = (
                df.groupby(["indicator_k_denominator", "indicator_k_denominator_new"])
                .size()
            ...
            all_denominator_counts = all_denominator_counts.append(
                denominator_match_counts
            )

            # df.to_feather(dirpath / file.name)

    all_denominator_counts.to_csv(
        dirpath / "new-indicator-k_denominator-match-counts.csv", index=False
    )
